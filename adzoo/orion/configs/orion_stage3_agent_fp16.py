...
use_memory = True
# [StyleDrive] FP16 closed-loop = the authors' validated low-memory path (README: ">17GB,
# almost the same performance as FP32"). fp16_infer loads the LLM + img_backbone in fp16.
fp32_infer=False
fp16_infer=True
num_gpus = 32
batch_size = 4
...
num_epochs = 6
llm_path = '/mnt/HDD5/college_student/Ourion/ckpts/pretrain_qformer/'
use_gen_token = True
use_col_loss = True
...
half_precision_load=True       # convert the model to half in agent.setup() to fit a 24G card
half_precision_dtype='float16'  # [StyleDrive] fp16 (not bf16) — matches fp16_infer + autocast
