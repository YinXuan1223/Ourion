...
# out_dir 必須指到 dow904_ckpt 內(只有它被 ACL 開放給 dow904 可寫);
# 通用 launcher orion_dist_train.sh 會把 checkpoint 存到 out_dir + basename(work_dir),
# work_dir basename = config 檔名 → /mnt/HDD6/yinxuan/dow904_ckpt/orion_stage2_vae_style_ft_resume/
checkpoint_config = dict(
    interval=2000, max_keep_ckpts=3, out_dir='/mnt/HDD6/yinxuan/dow904_ckpt')
log_config = dict(
    interval=10,
...
load_from = '/mnt/HDD5/college_student/Ourion/ckpts/Orion.pth' # 如果要從 pretrain ckpt 繼續訓練,就改成 pretrain ckpt 的路徑
resume_from = '/mnt/SSD7/college_student/outputs/orion_stage2_vae_style_ft_frozen/iter_4000.pth' # 如果要 resume,就改成上次訓練的 checkpoint
workflow = [('train', 1)]
