...
use_memory = True
# [StyleDrive] bf16 weights so the 7.5B model fits a 24G card (matches the eval path).
# forward() always runs under autocast(bf16); fp32 weights would OOM at .cuda().
fp32_infer=False
num_gpus = 32
batch_size = 4
num_iters_per_epoch = 234769 // (num_gpus * batch_size)
num_epochs = 6
llm_path = '/mnt/HDD5/college_student/Ourion/ckpts/pretrain_qformer/'  # absolute: closed-loop cwd is Bench2Drive/
use_gen_token = True
use_col_loss = True
...
model = dict(
    ...
    # [StyleDrive] closed-loop with the trained diffusion + style planner (Run 1: token mode).
    use_diff_decoder = True,
    plan_anchor_path = '/mnt/HDD5/college_student/Ourion/data/others/b2d_plan_anchor_20mode.pkl',
    ego_fut_mode = 20,
    ...
    use_style_conditioning = True,   # [StyleDrive] read driving_style at inference
    style_inject_mode = 'token',     # [StyleDrive] must match the trained ckpt (Run 1)
...
data_root = "/mnt/HDD5/college_student/Ourion/data/bench2drive"
info_root = "/mnt/HDD5/college_student/Ourion/data/infos"
map_root = "/mnt/HDD5/college_student/Ourion/data/bench2drive/maps"
map_file = "/mnt/HDD5/college_student/Ourion/data/infos/b2d_map_infos.pkl"
...
# [StyleDrive] load weights in bf16 so the model fits a 24G GPU (mirrors test.py).
# The agent (orion_b2d_agent.py setup()) honors this flag before .cuda().
