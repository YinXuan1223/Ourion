...
num_epochs = 3
ft_max_iters = 4000   # [StyleDrive] short frozen fine-tune (override runner max_iters)
...
model = dict(
    ...
    use_diff_decoder = True,  # [StyleDrive] enable diffusion decoder
    plan_anchor_path = 'data/others/b2d_plan_anchor_20mode.pkl',
    ego_fut_mode = 20,
    ...
    plan_cls_loss_smooth = True,
    use_style_conditioning = True,  # [StyleDrive] enable style conditioning
    style_inject_mode = 'token',    # [StyleDrive] inject style as a token into ego_query cross-attention
    style_dropout = 0.15,           # [StyleDrive] zero style 15% of the time -> enables CFG
    train_only_planner = True,      # [StyleDrive] freeze everything except diffusion planner
...
# [StyleDrive] frozen fine-tune data: set to mixed (run1) or annotated-only (run2) via --cfg-options
ann_file_train=info_root + f"/b2d_infos_train_style_annotated.pkl"  # [StyleDrive] Run2: annotated-only
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D',\
         keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'ego_his_trajs','input_ids','gt_attr_labels', 'ego_fut_trajs', 'ego_fut_masks','ego_fut_cmd', 'ego_lcf_feat','vlm_labels','can_bus', 'traffic_state_mask', 'traffic_state', 'driving_style']+collect_keys),  # [StyleDrive] added driving_style
]
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='CustomCollect3D',\
                keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'ego_his_trajs','input_ids','gt_attr_labels', 'ego_fut_trajs', 'ego_fut_masks','ego_fut_cmd', 'ego_lcf_feat','vlm_labels','can_bus','fut_valid_flag', 'driving_style']+collect_keys,  # [StyleDrive] collect driving_style at eval for style sweep
                )]
    )
]
...
# [StyleDrive] use the registered LearningRateDecayOptimizerConstructor — it SKIPS
# requires_grad=False params (layer_decay_optimizer_constructor.py:106), so with the backbone
# frozen only the trainable planner params enter the optimizer. (plain AdamW failed: this fork's
# registry has no DefaultOptimizerConstructor.)
...
evaluation = dict(interval=ft_max_iters*10, pipeline=test_pipeline) # no eval
find_unused_parameters=False  # frozen params skipped by DDP; planner fully used
# [StyleDrive] checkpoints (~18G each) go to SSD7 because HDD5 is full;
# longer interval + keep only 2 => peak ~36G on SSD7. Final path:
# /mnt/SSD7/college_student/outputs/orion_stage2_diff_style_train/
# checkpoint_config = dict(interval=5000, max_keep_ckpts=3)  # original
checkpoint_config = dict(interval=1000, max_keep_ckpts=3,
                         out_dir='/mnt/SSD7/college_student/outputs')
runner = dict(
    type='IterBasedRunner', max_iters=ft_max_iters)
log_config = dict(
    interval=10, hooks=[dict(type="TextLoggerHook"), dict(type="TensorboardLoggerHook")]
)

half_precision_load=True
# [StyleDrive] frozen fine-tune starts from the trained diffusion+style model.
# token-mode style_token_proj is missing in this ckpt -> zero-init (identity start);
# old 'add'-mode style_proj keys are ignored. diff_decoder/plan_anchor_encoder/time_mlp load.
load_from='/mnt/SSD7/college_student/outputs/orion_stage2_diff_style_train/iter_117384.pth'
resume_from=None